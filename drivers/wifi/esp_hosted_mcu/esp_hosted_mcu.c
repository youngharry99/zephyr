...
/* One device carries both interfaces, so resolve the interface from the net_if. */
#if defined(CONFIG_WIFI_ESP_HOSTED_MCU_AP_STA_MODE)
NET_IF_DT_INST_DECLARE(0, 1);
#define esp_hosted_mcu_iface_ap NET_IF_DT_INST_GET(0, 1)
#endif

static inline size_t esp_hosted_mcu_iface_index(struct net_if *iface)
{
#if defined(CONFIG_WIFI_ESP_HOSTED_MCU_AP_STA_MODE)
	if (iface == esp_hosted_mcu_iface_ap) {
		return ESP_HOSTED_MCU_IFACE_AP;
	}
#endif
	ARG_UNUSED(iface);

	return ESP_HOSTED_MCU_IFACE_STA;
}
				  ...
				  struct wifi_connect_req_params *params)
{
	...
	if (params->psk_length) {
		...
		/* Open network: no key material to copy. */
	}
				    ...
				    struct wifi_connect_req_params *params)
{
	...
	authmode = esp_hosted_mcu_ap_authmode(params->security);
	if (authmode < 0) {
		LOG_ERR("unsupported AP security %d", params->security);
		ret = authmode;
		goto out;
	}
...
/* Fill in the soft-AP details the coprocessor holds in its AP config. */
static int esp_hosted_mcu_ap_status(struct wifi_iface_status *status)
{
	Rpc req = Rpc_init_zero;
	Rpc resp = Rpc_init_zero;
	const wifi_ap_config *ap;
	int ret;

	req.msg_id = RpcId_Req_WifiGetConfig;
	req.which_payload = Rpc_req_wifi_get_config_tag;
	req.payload.req_wifi_get_config.iface = ESP_HOSTED_MCU_WIFI_IF_AP;

	ret = esp_hosted_mcu_rpc_call(&req, &resp, ESP_HOSTED_MCU_RPC_TIMEOUT);
	if (ret || !resp.payload.resp_wifi_get_config.has_cfg) {
		pb_release(Rpc_fields, &resp);
		return 0;
	}

	ap = &resp.payload.resp_wifi_get_config.cfg.u.ap;
...
{
	struct esp_hosted_mcu_data *data = dev->data;
	size_t itf = esp_hosted_mcu_iface_index(iface);
	Rpc req = Rpc_init_zero;
	Rpc resp = Rpc_init_zero;
	...
	/*
	 * The state field carries the soft-AP enum for the AP interface and the
	 * station enum for the station one.
	 */
	if (itf == ESP_HOSTED_MCU_IFACE_AP) {
		status->state = (data->state[itf] == WIFI_STATE_COMPLETED)
					? WIFI_SAP_IFACE_ENABLED
					: WIFI_SAP_IFACE_DISABLED;
		return esp_hosted_mcu_ap_status(status);
	}

	status->state = data->state[itf];

	/* Only a connected station can report the associated AP details. */
	if (data->state[itf] != WIFI_STATE_COMPLETED) {
		return 0;
	}
...
static int esp_hosted_mcu_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	size_t itf = esp_hosted_mcu_iface_index(net_pkt_iface(pkt));
...
NET_DEVICE_DT_INST_DEFINE(0, esp_hosted_mcu_wifi_init, NULL, &esp_hosted_mcu_data_0, NULL,
			  CONFIG_WIFI_INIT_PRIORITY, &esp_hosted_mcu_api, ETHERNET_L2,
