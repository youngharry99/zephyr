...
void riscv_clic_irq_priority_set(uint32_t irq, uint32_t pri, uint32_t flags)
{
	...
	uint32_t lnlbits = (clic_info & CLIC_INFO_CLICINTCTLBITS_Msk) >> CLIC_INFO_CLICINTCTLBITS_Pos;
	CLIC_CTRL[irq].INTCTRL = ((long unsigned int)CLIC_CTRL[irq].INTCTRL & (~CLIC_INTCFG_PRIO_Msk)) | (pri << (8 - lnlbits));
...
{
	CLIC_MTH.w = 0;
	CLIC_CFG.w = (((CLIC_INFO.qw & CLIC_INFO_CLICINTCTLBITS_Msk) >> CLIC_INFO_CLICINTCTLBITS_Pos) << CLIC_CLICCFG_NLBIT_Pos);
