...
/* SDIO rounds a short transfer up to the next word, SPI DMA needs the same. */
BUILD_ASSERT(ESP_HOSTED_MCU_FRAME_SIZE % 4 == 0,
	     "esp-hosted-mcu frame size must be a multiple of 4 bytes");

/*
 * Alignment for any buffer a transport may hand to DMA. Cache maintenance
 * around a DMA receive operates on whole cache lines: an invalidate that
 * covers a line shared with another object discards that object's dirty
 * bytes on a write-back cache. Align to a cache line where the host has a
 * data cache, and to a word otherwise.
 */
#if defined(CONFIG_DCACHE) && (CONFIG_DCACHE_LINE_SIZE > 0)
#define ESP_HOSTED_MCU_DMA_ALIGN CONFIG_DCACHE_LINE_SIZE
#else
#define ESP_HOSTED_MCU_DMA_ALIGN 4
#endif

/* On an Espressif host the transport ISR must run with the flash cache off. */
#ifdef CONFIG_SOC_FAMILY_ESPRESSIF_ESP32
#include <esp_attr.h>
#define ESP_HOSTED_MCU_ISR_ATTR IRAM_ATTR
#else
#define ESP_HOSTED_MCU_ISR_ATTR
#endif
...
struct esp_hosted_mcu_transport_api {
	...
	/*
	 * Block up to timeout for the coprocessor to signal new receive data,
	 * returning early on the transport's data-ready interrupt. May wake
	 * spuriously; the caller must re-check data_ready(). A transport with
